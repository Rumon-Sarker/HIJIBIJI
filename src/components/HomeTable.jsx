...
import { getContactData, getHomeContactData } from "@/controls/fetchData/fetchData";
...
const HomeTable = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response =
          filter === "customDate"
            ? await getHomeContactData({ fromDate, toDate })
            : await getHomeContactData(filter);
  ...
  return (
    <div>
      <Toaster />
      <div>
        <h1 className="text-center text-main text-2xl my-5">From Home Contact</h1>
