...
import { getContactData, getFooterContactData } from "@/controls/fetchData/fetchData";
...
const FooterTable = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response =
          filter === "customDate"
            ? await getFooterContactData({ fromDate, toDate }) 
            : await getFooterContactData(filter);
  ...
  return (
    <div>
      <Toaster />
      <div>
        <h1 className="text-center text-main text-2xl my-5">From Footer Contact</h1>
