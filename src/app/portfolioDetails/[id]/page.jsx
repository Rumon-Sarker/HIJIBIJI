...
const PortfolioDetails = async ({ params }) => {
  ...
  const { name, categoryId, id, image, description, scope, createdAt } = res;

  dayjs.extend(advancedFormat);
  const formattedDate = dayjs(createdAt).format("dddd - MMMM Do, YYYY");

  const getFilteredItems = (data, categoryId, excludedIds) => {
    return data
      .filter(
        (item) => item.categoryId === categoryId && item.id !== excludedIds
      )
      .slice(0, 3);
  };

  const relatedItems = getFilteredItems(data, categoryId, id);
